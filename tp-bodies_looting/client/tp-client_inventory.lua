...
RegisterNetEvent('tp-bodies_looting:openBasedUI')
AddEventHandler('tp-bodies_looting:openBasedUI', function(_data, coords)
...
function loadPlayerInventory(targetSource)
    ...
    ESX.TriggerServerCallback("tp-bodies_looting:getPlayerInventory",function(data)
            items = {}
    ...
    if inventory ~= nil then
        for key, value in pairs(inventory) do
            ...
            if value.count > 0 then
                ...
                if not Config.BlackListedItems[value.name] then
                    inventory[key].type = "item_standard"
                    table.insert(items, inventory[key])
                end
            end
    ...
    if weapons ~= nil then
        for key, value in pairs(weapons) do
            ...
            if  weapons[key].label ~= "NOT_AVAILABLE" then
                ...
                if not Config.BlackListedWeapons[string.upper(weapons[key].name)] then
...
RegisterNUICallback(
    ...
    function(data, cb)
        ...
        if type(data.number) == "number" and math.floor(data.number) == data.number then
            ...
            TriggerServerEvent("tp-bodies_looting:tradePlayerItem", currentBagCoords, data.item.type, data.item.name, count, data.item.count)
        end
    
        Wait(250)
        ESX.TriggerServerCallback("tp-bodies_looting:getOtherInventory",function(bag_data)
